...
void FastPlannerManager::initPlanModules(
  ...
  nh.param("odometry_topic", odom_topic, std::string("/quad_0/lidar_slam/odom"));
...
bool FastPlannerManager::checkTrajTrackingError(double &rmse) {
  // Skip check if trajectory hasn't been initialized yet
  if (local_data_.traj_id_ <= 1) {
    rmse = 0.0;
    return true;
  }
  ...
  if (curr_time < 0 || curr_time > local_data_.duration_) {
    rmse = 0.0;
    return true;
  }
  
  // Skip checking for the first 0.5 seconds to allow trajectory startup
  // This prevents false positives when transitioning to a new trajectory
  const double startup_grace_period = 0.5;
  if (curr_time < startup_grace_period) {
    rmse = 0.0;
    ROS_DEBUG("Skipping tracking check: t=%.3f < %.3f (grace period)", 
              curr_time, startup_grace_period);
    return true;
  }
  
  // Get desired position from trajectory
  Vector3d desired_pos = traj.getPos(curr_time);
  
  // Get current actual position from odometry
  Vector3d actual_pos = local_data_.curr_pos_;
  
  // Calculate tracking error
  Vector3d error = desired_pos - actual_pos;
  rmse = error.norm();
  
  // Debug output with more details
  ROS_DEBUG("Tracking check: curr_time=%.3f, traj_id=%d, desired=[%.2f,%.2f,%.2f], actual=[%.2f,%.2f,%.2f], error=%.3f",
            curr_time, local_data_.traj_id_, desired_pos.x(), desired_pos.y(), desired_pos.z(),
            actual_pos.x(), actual_pos.y(), actual_pos.z(), rmse);
  
  // Check against configured threshold
  if (rmse > gcopter_config_->maxTrackingError) {
    ROS_WARN("Tracking error too large: %.3f m (threshold: %.3f m) at t=%.3f (traj_id=%d, start=%.3f, now=%.3f)", 
             rmse, gcopter_config_->maxTrackingError, curr_time, 
             local_data_.traj_id_, local_data_.start_time_.toSec(), ros::Time::now().toSec());
    return false;
  }
...
bool FastPlannerManager::planExploreTraj(const vector<Eigen::Vector3f> &path,
  ...
  if (is_static) {
    ...
    // Always use actual current position to avoid accumulated tracking error
    // If we use predicted position from old trajectory, large errors can accumulate
    time_now =
        time_now > local_data_.duration_ ? local_data_.duration_ : time_now;
    Eigen::Vector3d current_pose = local_data_.curr_pos_;  // Use actual position
    ...
    ROS_DEBUG("Replan: using actual pos=[%.2f,%.2f,%.2f] vs predicted=[%.2f,%.2f,%.2f]",
              current_pose.x(), current_pose.y(), current_pose.z(),
              local_data_.minco_traj_.getPos(time_now).x(),
              local_data_.minco_traj_.getPos(time_now).y(),
              local_data_.minco_traj_.getPos(time_now).z());
  }
